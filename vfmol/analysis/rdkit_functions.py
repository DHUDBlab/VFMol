...
class BasicMolecularMetrics(object):
    def __init__(self, dataset_info, train_smiles=None, args=None):
        ...
        self.cond_val = MeanAbsoluteError()  # 平均绝对误差
    ...
    def compute_validity(self, generated):
        ...
        for graph in tqdm(generated, desc="Generated molecules validity check progress"):
            ...
            if smiles is not None:
                try:
                    mol_frags = Chem.rdmolops.GetMolFrags(  # 拆解分子成所有的“分子片段”（即连接的部分）
                        mol, asMols=True, sanitizeFrags=True
                    )
                    largest_mol = max(  # 找出最大片段（原子数最多）
                        mol_frags, default=mol, key=lambda m: m.GetNumAtoms()
                    )
                    smiles = mol2smiles(largest_mol)
                    all_smiles.append(smiles)
                except Chem.rdchem.AtomValenceException:  # 原子价错误（Valence）
                    print("Valence error in GetmolFrags")
                    all_smiles.append(None)
                except Chem.rdchem.KekulizeException:  # 芳香化失败（Kekulization）
                    print("Can't kekulize molecule")
                    all_smiles.append(None)
            else:
                all_smiles.append(None)

        return num_valid / len(generated), np.array(num_components), all_smiles
    ...
    def compute_relaxed_validity(self, generated, test=False):
        valid = []
        all_smiles = []
        num_valid = 0
        for graph in tqdm(generated, desc="Generated molecules relaxed validity check progress"):
            atom_types, edge_types = graph
            mol = build_molecule_with_partial_charges(  # 允许使用**partial charges（部分电荷）**来构建分子
        ...
        if test:
            with open(r"final_smiles_relaxed.txt", "w") as fp:
                for smiles in all_smiles:
                    fp.write("%s\n" % smiles)
                print("All smiles saved")

        return valid, num_valid / len(generated), all_smiles

    def cond_sample_metric(self, samples, input_properties):
        # samples: 一个列表，每个元素是 (atom_types, edge_types)，表示一个生成的分子图。
        # input_properties: 条件生成时的输入目标属性值，比如期望的 dipole、HOMO 或两者。
        mols_dipoles = []
        mols_homo = []
        ...
        for i, sample in enumerate(samples):
            ...
            mol = Chem.AddHs(mol)
            params = ETKDGv3()  # 通过 RDKit 生成 3D 构象（ETKDG → MMFF 优化）
            params.randomSeed = 1
            try:
    ...
    def evaluate(self, generated, input_properties=None, test=False):
        ...
        input_properties：如果是条件生成任务，这里是参考的分子性质值。
        test：如果设为 True，会将生成的 SMILES 保存到 final_smiles.txt 文件中。"""
        validity, num_components, all_smiles = self.compute_validity(generated)
        ...
        if test:
            ...
            print("all_smiles first 100:", all_smiles[:100])
        ...
        print(
            f"Number of connected components of {len(generated)} molecules: min:{nc_min:.2f} mean:{nc_mu:.2f} "
            f"max:{nc_max:.2f}")

        relaxed_valid, relaxed_validity, all_smiles_relaxed = self.compute_relaxed_validity(generated, test=test)
        print(
            f"Relaxed validity over {len(generated)} molecules: {relaxed_validity * 100 :.2f}%"
        )

        cond_mae = cond_val = -1.0
        if input_properties is not None:  # 条件生成评估
            cond_mae, cond_val = self.cond_sample_metric(generated, input_properties)

        if relaxed_validity > 0:  # （使用 relaxed valid 分子）
            unique, uniqueness = self.compute_uniqueness(relaxed_valid)  # all_smiles_relaxed
            print(
...
def build_molecule_with_partial_charges(atom_types, edge_types, atom_decoder):
    # 尝试自动修复轻微的价态不一致，
    # 容忍模型生成的一些“不完美结构”，
    # 尝试在无法 kekulize、无法识别 SMILES 的情况下，尽量构建出 RDKit 可以接受的分子对象
    ...
    edge_types = torch.triu(edge_types)  # 保留上三角，避免重复加边
    edge_types[edge_types >= 5] = 0  # set edges in virtual state to non-bonded
    all_bonds = torch.nonzero(edge_types)

    for i, bond in enumerate(all_bonds):
        if bond[0].item() != bond[1].item():  # 判断是否是对角线（自己跟自己）——跳过
            mol.AddBond(
                bond[0].item(),
                bond[1].item(),
                bond_dict[edge_types[bond[0], bond[1]].item()],
            ...
            flag, atomid_valence = check_valency(mol)  # 价态检查与“轻微修复”
            ...
            if flag:
                ...
                # 只处理：N (7), O (8), S (16) 这几类原子（常见可带正电的元素）
                if an in (7, 8, 16) and (v - ATOM_VALENCY[an]) == 1:
                    mol.GetAtomWithIdx(idx).SetFormalCharge(1)
                    # print("Formal charge added")
                # elif an in (7, 8, 16) and (v - ATOM_VALENCY[an]) == -1:  # 也许？
                #     mol.GetAtomWithIdx(idx).SetFormalCharge(-1)

                # 比如模型生成了一个 N 的价态为 4（应为 3），它可能是 N+ => 手动设置为带正电的氮 N+
                # 不处理负电荷、不处理更复杂价态错误（比如 NH+、[N-]）
    return mol
...
def compute_molecular_metrics(
    ...
    validity_dict = {"mol_stable": -1, "atm_stable": -1}  # 分子整体稳定性，原子级别的稳定性
