...
class DummyExtraFeatures:
    def __init__(self):
        ...
        当你不想添加额外特征时，返回全零且维度合法的占位张量"""

    def __call__(self, noisy_data):
        # 让 DummyExtraFeatures 实例可以像函数一样被调用
        # eg：dummy = DummyExtraFeatures()
        # out = dummy(noisy_data)
        X = noisy_data["X_t"]
        E = noisy_data["E_t"]
        y = noisy_data["y_t"]
        empty_x = X.new_zeros((*X.shape[:-1], 0))  # (batch, n, 0)
        empty_e = E.new_zeros((*E.shape[:-1], 0))  # (batch, n, n, 0)
        empty_y = y.new_zeros((y.shape[0], 0))  # (batch, 0)
...
class ExtraFeatures:
    ...
    def __call__(self, noisy_data):
        n = noisy_data["node_mask"].sum(dim=1).unsqueeze(1) / self.max_n_nodes  # “节点数量比例”，(bs, 1)
        ...
        if self.features_type == "cycles":
            ...
            rrwp_edge_attr = self.RRWP(E, k=self.rrwp_steps)  # bs, n, n, k

            # 提取每个节点的 RRWP(u,u,t) 自循环概率（从自身走 t 步又回到自己的概率）-> 反映节点的“自稳定性”、“结构对称性”等性质
...
class RRWPFeatures:
    # Row-normalized Random Walk Polynomial 是一种基于**随机游走（Random Walk）**的图结构特征。
    # 表示每一阶（第 t 层）从每个节点到另一个节点的 t 步“到达概率”
    ...
    def __call__(self, E, k=None):
        ...
        to_fill = 1 / (E.sum(dim=-1).float())  # D^-1
        to_fill[E.sum(dim=-1).float() == 0] = 0  # 防止除以 0
        degree = torch.diagonal_scatter(degree, to_fill, dim1=1, dim2=2)  # 构造 D^-1
        E = degree @ E  # 计算随机游走矩阵 P = D^-1 @ A

        id = torch.eye(n, device=E.device).unsqueeze(0).repeat(bs, 1, 1)
        rrwp_list = [id]  # P^0 = I

        for i in range(k - 1):
            cur_rrwp = rrwp_list[-1] @ E  # 每一阶 P^t = P^{t-1} * P
            rrwp_list.append(cur_rrwp)

        return torch.stack(rrwp_list, -1)  # 输出形状 (bs, n, n, k)


class NodeCycleFeatures:
    def __init__(self):
        # 从图的邻接矩阵中提取每个节点的环结构特征，并对它们进行归一化和掩码处理，作为节点或图的结构性输入特征
        self.kcycles = KNodeCycles()

    def __call__(self, noisy_data):
        adj_matrix = noisy_data["E_t"][..., 1:].sum(dim=-1).float()  # 得到邻接矩阵 bs,n,n

        x_cycles, y_cycles = self.kcycles.k_cycles(
            adj_matrix=adj_matrix
        )  # (bs, n_cycles)
        # x_cycles 是每个 节点 的 k-cycle 向量，shape: (bs, n, 3)，对应 3/4/5 元环的数量
        # y_cycles 是每个 图整体 的 k-cycle 向量，shape: (bs, 4)，对应 3~6 元环的总数
        x_cycles = x_cycles.type_as(adj_matrix) * noisy_data["node_mask"].unsqueeze(-1)
        # 只保留真实节点的 cycle 特征，把 padding 的节点设为 0

        # Avoid large values when the graph is dense 防止过大
        x_cycles = x_cycles / 10
        y_cycles = y_cycles / 10
        x_cycles[x_cycles > 1] = 1  # Clamp 截断到 [0, 1]
...
class EigenFeatures:
    ...
    从图的拉普拉斯矩阵中提取图的谱特征（eigen features），包括特征值和特征向量
    """

    def __init__(self, mode):
        """mode: 'eigenvalues' or 'all'"""
        self.mode = mode  # mode='all' 提取特征值和特征向量
    ...
    def __call__(self, noisy_data):
        ...
        L = compute_laplacian(A, normalize=False)  # 计算拉普拉斯矩阵 L
        # 屏蔽掉 padding 部分 ，对 padding 的位置填上大的数值，避免在求特征值时扰动真实图结构
        ...
        if self.mode == "eigenvalues":
            ...
            eigvals, eigvectors = torch.linalg.eigh(L)  # 特征值信息，特征向量信息
            # 特征值（eigenvalues）特征：用于衡量图的全局结构（比如连通分量数等）。
            # (bs, n)，每个图的 n 个特征值
            # 特征向量（eigenvectors）特征：用于识别节点在图中的结构角色，尤其是连通组件信息。
            # (bs, n, n)，每个图的 n 个特征向量（每列是一个特征向量）

            # print(eigvals)
            # 特征值做归一化，除以图中实际存在的节点数。保证不同大小图的特征值可比
            eigvals = eigvals.type_as(A) / torch.sum(mask, dim=1, keepdim=True)
            eigvectors = eigvectors * mask.unsqueeze(2) * mask.unsqueeze(1)
            # Retrieve eigenvalues features
            # n_connected_comp：每个图的连通子图个数（即特征值为 0 的个数）。
            # batch_eigenvalues：每个图中前 k 个非零特征值。
            n_connected_comp, batch_eigenvalues = get_eigenvalues_features(
                eigenvalues=eigvals
            )

            # Retrieve eigenvectors features
            # nonlcc_indicator：每个图中 不属于最大连通子图的节点 的 indicator（1 表示不属于）。
            # k_lowest_eigenvector：每个图中最小的 k 个非零特征值对应的特征向量（用于描述 LCC 中节点的结构）
            ...
            return (
                n_connected_comp,  # 每个图的连通子图数量（可表示图是否断开）  bs，
                batch_eigenvalues,  # 每个图的最小非零特征值（表示图的稠密程度、连通度）
                # 从拉普拉斯特征值中取出前 k 个最小非零值（去掉 0 对应的特征值，因为 0 特征值的个数 == 连通子图数）
                nonlcc_indicator,  # 哪些节点不在最大连通子图中（用于 mask 或分析结构） n
                k_lowest_eigenvector,  # 最有意义的特征向量（结构描述） n，k
            )
        else:
...
def get_eigenvectors_features(vectors, node_mask, n_connected, k=2):
    ...
    indices = (torch.arange(k).type_as(vectors).long().unsqueeze(0).unsqueeze(0)
...
class KNodeCycles:
    ...
    计算图中每个节点参与的循环（cycle）数量
    提取每个节点在 三元环、四元环、五元环 中的出现次数（局部）
    提取整个图中 三～六元环 的总数（全局）
    Aii 3：节点 i 到自身的长度为 3 的路径条数（和三角形有关）
    trace(Ak)：图中 k 长度闭环的路径总数（每个环会被多次计入）
    """
    def __init__(self):
        super().__init__()
    ...
    def k_cycles(self, adj_matrix):
        ...
        kcyclesx = torch.cat([k3x, k4x, k5x], dim=-1)  # 每个节点参与的cycle数
        kcyclesy = torch.cat([k3y, k4y, k5y, k6y], dim=-1)  # 图中总共的cycle数
    ...
    def k3_cycle(self):
        ...
        # c3 / 2: 每个节点参与的三角形数（按无向边对称去重）。 sum(c3) / 6: 图中总共的三角形数
        return (c3 / 2).unsqueeze(-1).float(), (torch.sum(c3, dim=-1) / 6).unsqueeze(
            -1).float()  # 这些路径是成对重复的，三角形会被数了 6 次

    def k4_cycle(self):
        diag_a4 = batch_diagonal(self.k4_matrix)
        c4 = (  # 去掉非环结构（如“链路”或“双边”）
                diag_a4
                - self.d * (self.d - 1)  # 表示节点 i 发出去的所有“走一步然后回来”的路径组合，
