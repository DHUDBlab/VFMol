...
class CEPerClass(Metric):
    # 多分类任务中对单个类别的预测质量进行精细评估
    # 追踪某一个具体类别的交叉熵性能指标的类，可以理解为 “Cross Entropy per Class”
    full_state_update = False  # 表示每次只更新当前 batch 的状态，而不是重新计算所有历史数据

    def __init__(self, class_id):
        super().__init__()
        # 记录某个类别（如 class_id = 2）的累计 BCE 损失和样本数（用于平均）
        ...
        self.binary_cross_entropy = torch.nn.BCELoss(reduction="sum")  # 二分类
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        """
        target = target.reshape(-1, target.shape[-1])  # bs*n, d
        mask = (target != 0.0).any(dim=-1)  # 过滤掉全0的位置（即无标签）
